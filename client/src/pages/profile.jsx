...
const ProductImage = ({ src, alt }) => {
  ...
  const [imgLoaded, setImgLoaded] = useState(() => loadedImageCache.has(resolvedSrc));
  const handleLoad = useCallback(() => { loadedImageCache.add(resolvedSrc); setImgLoaded(true); }, [resolvedSrc]);
  return (
    <div className="relative h-48 w-full bg-gray-200">
      ...
      <img src={resolvedSrc} alt={alt} className={`h-48 w-full object-cover transition-opacity duration-300 ${imgLoaded ? "opacity-100" : "opacity-0"}`} onLoad={handleLoad} />
    </div>
  );
};
...
export default function Profile() {
  ...
  const [formData, setFormData] = useState({ full_name: "", address: "", contact_number: "" });
  const [tempFormData, setTempFormData] = useState({ full_name: "", address: "", contact_number: "" });
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      if (data?.user) { setUser(data.user); fetchAllUserData(data.user.id); }
    };
    getUser();
  }, [fetchAllUserData]);
  ...
  useEffect(() => {
    if (!user) return;
    const ch = supabase.channel('profile-orders-watch')
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'orders', filter: `seller_id=eq.${user.id}` },
        () => { supabase.rpc("get_seller_sold_count", { p_seller_id: user.id }).then(({ data }) => { if (data != null) setSoldCount(data); }); })
      .subscribe();
  ...
  const isProfileComplete = useMemo(() => profile?.address && profile?.contact_number, [profile?.address, profile?.contact_number]);
  ...
  const handleAddProductClick = useCallback(() => {
    if (!isProfileComplete) {
      ...
      setFormData({ full_name: profile?.full_name || "", address: profile?.address || "", contact_number: profile?.contact_number || "09" });
      setContactError("");
      setIsEditing(true);
      return;
    }
  ...
  const handleContactNumberChange = useCallback((e) => {
    ...
    if (numericValue.length < 2 || !numericValue.startsWith("09")) numericValue = "09" + numericValue.replace(/^09/, "");
    numericValue = numericValue.slice(0, 11);
    setFormData((prev) => ({ ...prev, contact_number: numericValue }));
    if (numericValue.length === 2) setContactError("Please enter the remaining 9 digits");
    else if (numericValue.length < 11) setContactError(`Contact number must be exactly 11 digits (${11 - numericValue.length} more digits needed)`);
  ...
  const handleUpload = async (e) => {
    try {
      ...
      const { data, error: uploadError } = await supabase.storage.from("avatars").upload(filePath, fileToUpload, { cacheControl: "3600", upsert: true });
  ...
  const handleProfileUpdate = async () => {
    if (contactError) { toast.error("Please fix contact number errors first"); return; }
    if (formData.contact_number.length !== 11) { toast.error("Contact number must be exactly 11 digits"); return; }
    try {
      ...
      setProfile((prev) => ({ ...prev, ...formData, avatar_url: tempFormData.avatar_url || profile?.avatar_url }));
      setIsEditing(false);
  ...
  const handleCancelEdit = () => {
    if (tempAvatarUrl) setProfile((prev) => ({ ...prev, avatar_url: tempAvatarUrl }));
    setFormData({ full_name: profile?.full_name || "", address: profile?.address || "", contact_number: profile?.contact_number || "09" });
    setTempFormData({});
    setContactError("");
    setIsEditing(false);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-green-50">
      <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="flex flex-col items-center md:flex-row md:items-start gap-6 md:gap-8">
            ...
            <div className="relative flex-shrink-0">
              <div className="relative w-32 h-32 sm:w-40 sm:h-40">
                <img src={profileImageUrl} alt="Profile" className="w-full h-full rounded-full object-cover border-4 border-white shadow-lg" />
                {uploading && (
              ...
              <button
                onClick={() => { setTempFormData({ full_name: profile?.full_name || "", address: profile?.address || "", contact_number: profile?.contact_number || "09" }); setFormData({ full_name: profile?.full_name || "", address: profile?.address || "", contact_number: profile?.contact_number || "09" }); setContactError(""); setIsEditing(true); }}
              ...
              >
                <Edit className="w-5 h-5" /><span className="font-medium">Edit Profile</span>
              </button>
            </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="relative">
                      <input type="text" value={formData.full_name} onChange={(e) => setFormData({ ...formData, full_name: e.target.value.slice(0, 25) })} maxLength={25} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent pr-12" placeholder="Enter your full name" />
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input type="text" value={formData.address} onChange={(e) => setFormData({ ...formData, address: e.target.value.slice(0, 35) })} maxLength={35} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent pr-12" placeholder="Enter your address" />
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input type="tel" value={formData.contact_number} onChange={handleContactNumberChange} className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent pr-12 ${contactError ? "border-red-500" : "border-gray-300"}`} placeholder="09XXXXXXXXX" />
        ...
        {/* Section tabs: Products | Order Requests */}
        <div className="flex bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6">
          <button
            onClick={() => setActiveSection('products')}
            className={`flex-1 flex items-center justify-center gap-2 py-3.5 font-semibold text-sm transition-colors border-b-2 ${
            ...
            <div className="flex items-center justify-between mb-4 sm:mb-6">
              ...
              <button
                ...
                className={`px-2 py-2 sm:px-4 sm:py-2 rounded-lg flex items-center justify-center gap-2 text-sm sm:text-base flex-shrink-0 transition-all duration-200 ${
                  isProfileComplete ? "bg-green-700 text-white hover:bg-green-800" : "bg-yellow-600 text-white hover:bg-yellow-700"
                }`}
                title={!isProfileComplete ? "Complete profile to add products" : "Add new product"}
              >
                <Plus className="w-4 h-4 sm:w-5 sm:h-5" />
                <span className="hidden sm:inline">{isProfileComplete ? "Add Product" : "Complete Profile"}</span>
              </button>
            </div>
              ...
              <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-yellow-700 text-sm mb-3">To start selling, please add the following information to your profile:</p>
                  ...
                  <div key={product.id} className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-md border border-white/20 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 overflow-hidden">
                    <div className="relative">
                      ...
                      <div className="absolute top-2 right-2 flex gap-1">
                        <button onClick={() => setEditingProduct(product)} className="bg-white/90 backdrop-blur-sm text-blue-500 hover:text-blue-600 p-2 rounded-full shadow-lg hover:bg-white transition-all duration-200" title="Edit Product">
                          <Edit3 className="w-4 h-4" />
                        </button>
                        <button onClick={() => setDeleteConfirm(product)} className="bg-white/90 backdrop-blur-sm text-red-500 hover:text-red-600 p-2 rounded-full shadow-lg hover:bg-white transition-all duration-200" title="Delete Product">
